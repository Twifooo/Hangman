...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Utilisation : go run main.go <fichier_de_mots> <nombre_de_lettres_revelees>")
		return
	}
	...
	if len(os.Args) > 2 {
		var err error
		nbLettresRevelees, err = strconv.Atoi(os.Args[2])
		if err != nil || nbLettresRevelees < 0 {
